...
describe("AvoFactory", () => {
  ...
  context("Multisig", async () => {
    ...
    describe("deploy", async () => {
      ...
      it("should deploy Avocado with index > 5", async () => {
        // must deploy at index 5 for that, from 5 on must be sequential
        await avoFactory.deploy(owner.address, 5);
        const result = await avoFactory.callStatic.deploy(owner.address, 6);
        expect(result).to.not.equal(constants.AddressZero);
      });

      it("should revert on deploy Avocado with index > 5 when index-1 not deployed yet", async () => {
        await avoFactory.deploy(owner.address, 4);
        await expect(avoFactory.deploy(owner.address, 6)).to.be.revertedWith("AvoFactory__IndexNonSequential");
      });
    ...
    describe("deployWithVersion", async () => {
      ...
      it("should deploy Avocado with index > 5", async () => {
        // must deploy at index 5 for that, from 5 on must be sequential
        await avoFactory.deployWithVersion(owner.address, 5, newAvocadoMultisigVersion);
        const result = await avoFactory.callStatic.deployWithVersion(owner.address, 6, newAvocadoMultisigVersion);
        expect(result).to.not.equal(constants.AddressZero);
      });

      it("should revert on deploy Avocado with index > 5 when index-1 not deployed yet", async () => {
        await avoFactory.deployWithVersion(owner.address, 4, newAvocadoMultisigVersion);
        await expect(avoFactory.deployWithVersion(owner.address, 6, newAvocadoMultisigVersion)).to.be.revertedWith(
